...
def getScore(state):
    score = 0                                               # Initialize the score as 0 in the begginning
...
#minmax algorithm
    MaxRange = len(moveList);                                        # Calculate the number of possible moves and store in a variable


    for i in range(MaxRange):                                        # look further ahead multiple moves possible until the timer runs out
        bestMoveSoFar = moveList[i];                                 # Overwrite the previous best Move with the new Best move
        if timeOut(startTime, timeLimit):                            # if the timer runs out then the deepest sought move is the best move so far and returned                       
            return bestMoveSoFar
            break;                                                   # Exit the loop once the timer runs out
