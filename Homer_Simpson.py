...
def getMove(state, hWidth, hHeight, timeLimit):
    ...
    if state.playerToMove == 0:                                      # Finally, pick the move with the worst score (Homer_Simpson's strategy)
        bestMoveSoFar = moveList[scoreList.index(min(scoreList))]    # If we are Player 1, we look for the minimum score
    else:
        bestMoveSoFar = moveList[scoreList.index(max(scoreList))]    # If we are Player 2, we look for the maximum score
